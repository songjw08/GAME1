...
namespace Akila.FPSFramework
{
    ...
    public class AccelerateVector2Processor : InputProcessor<Vector2>
    {
        ...
        public override Vector2 Process(Vector2 value, InputControl control)
        {
            ...
            if (value.x != 0)
            {
                currentSpeedX += accelerationFactor * Time.deltaTime * value.x;
                currentSpeedX = Mathf.Clamp(currentSpeedX, -maxSpeedX, maxSpeedX); // Cap the speed if needed
            }
            else
            {
                // Decelerate the speed when there's no input
                currentSpeedX = Mathf.MoveTowards(currentSpeedX, 0, accelerationFactor * Time.deltaTime * deaccelerationFactor);
            }

            if (value.y != 0)
            {
                currentSpeedY += accelerationFactor * Time.deltaTime * value.y;
                currentSpeedY = Mathf.Clamp(currentSpeedY, -maxSpeedY, maxSpeedY); // Cap the speed if needed
            }
            else
            {
                // Decelerate the speed when there's no input
